...
//******* Custom Code (Delete if something is broken, probably my fault) ******/
    //add mask for outer edge of circle
    var mask = svg.append("defs")
            .append("mask")
            .attr("id", "botMask");

            // // add code for the shape of the mask
    mask.append('rect')
            .attr('x', 0)
            .attr('y', 0)
            .attr('width', svgWidth)
            .attr('height', svgHeight*.73)
            .style('fill', 'white');

//******* END Custom Code (One more line below) ******/
